...
export function AISearch({ className }: { className?: string }) {
  ...
  const showPanel = isOpen && (isLoading || response || error || query.length === 0);
  ...
  return (
    <div ref={panelRef} className={cn("relative", className)}>
      ...
      {/* Dropdown Panel */}
      {showPanel && (
        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-[600px] max-w-[95vw] bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden z-[200] max-h-[80vh] flex flex-col">
          ...
          <div className="sticky top-0 z-10 bg-white px-4 pt-4 pb-3 border-b border-slate-50">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Ask AI — find venues, photographers..."
                autoFocus
                className="w-full pl-14 pr-10 py-3 bg-slate-50 border border-[#668c65]/20 ring-4 ring-[#668c65]/5 rounded-2xl text-sm focus:outline-none transition-all"
              />
              {query && (
                <button type="button" onClick={() => { setQuery(""); setResponse(null); }}
                  className="absolute right-3 top-1/2 -translate-y-1/2 h-6 w-6 flex items-center justify-center rounded-full hover:bg-slate-200 transition-colors">
                  <X className="h-3.5 w-3.5 text-slate-400" />
                </button>
              )}
            ...
            <div className="divide-y divide-slate-50">
              ...
              {response.services.length > 0 && (
                <div className="p-3 space-y-2">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 px-2 pb-1">Results</p>
                    ...
                    <button key={svc.id} onClick={() => { router.push(`/services/${svc.id}`); setIsOpen(false); }}
                      ...
                      <div className="h-12 w-12 rounded-xl overflow-hidden bg-slate-100 flex-shrink-0">
                        {svc.thumbnail
                          ...
                          : <div className="w-full h-full bg-gradient-to-br from-[#668c65]/20 to-[#668c65]/40 flex items-center justify-center text-[#668c65] text-lg font-serif italic">{svc.name[0]}</div>
                        }
                      </div>
                      {/* Info */}
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-0.5">
                          ...
                          {svc.rating > 0 && <span className="flex items-center gap-1 text-[10px] text-amber-500"><Star className="h-3 w-3 fill-current" />{svc.rating.toFixed(1)}</span>}
                        </div>
                        {svc.price_range_min && (
                  ...
                  {/* View all */}
                  <button onClick={() => { router.push("/services"); setIsOpen(false); }}
                    className="w-full text-center py-3 text-xs font-bold text-[#668c65] hover:bg-[#668c65]/5 rounded-2xl transition-colors">
                    View all services →
                  </button>
                </div>
              )}
